...
| P-06 | **Re-scoped by D-031; Stage B COMPLETE 2026-08-04.** The OA theory pair is page-verified — Whitney's own MIT OCW 2.875 lecture (51 slides read; wedging θ > c/μ, jamming parallelogram, RCC, and theory-vs-data validation figures slides 35–38) + CJME 2025 review (CC BY; Whitney-model equations and jamming diagram, pp. 8–12). Caveats in C-12, INTERFACE_SPEC §2.3, and H04 (Addendum A5) are revised per their written terms. **Residual optional ask: the 1982 full text** (DOI 10.1115/1.3149634), now narrowed to the paper's **full experimental methodology only** (the OCW deck already reproduces the validation figures) | only if a T5-promotion argument needs the 1982 experimental methodology (Phase 1/2) | Struck entirely only if the 1982 full text itself lands |
