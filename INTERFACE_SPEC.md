...
| 18 | Insertion incomplete at encounter budget (added 2026-08-08, D-033) | Attempt reached the capture plane; the insertion window ended on the encounter time/attempt budget (contact-stage timeout) with no latch confirm and no other §8 signature. Covers budget-truncated insertion windows and sub-threshold shallow stalls (forces below the #62 jam thresholds — the #62 recalibration hook's territory) | Distinct outcome class — never folded into row 10 ("full stroke, no confirm" is a latch-mechanism signature, not a budget artifact) nor clean_miss (the plane was crossed / contact occurred, D-030 (b)/(c)). Response: retry per D-005 while budget remains; escalate on exhaustion |
